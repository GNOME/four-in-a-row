...
void gfx_draw_tile(gint row, gint col, gint tile_selector, gboolean do_refresh)
{
...
static void gfx_draw_all_tiles(void)
{
	gint row, col;
...
static gint gfx_timeout_animate(gpointer data)
{
...
{
	/* Drop current_player's counter into column col, returning the row it landed in */
...
static void gfx_draw_line(gint r1, gint c1, gint r2, gint c2, gint tile)
{
	gint d_row    = 0;
	gint d_col    = 0;
	gboolean done = FALSE;
...
static gint gfx_timeout_blink_line(gpointer data)
{
	Anim *this_anim = (Anim *)data;
	static gint tile = -1;
...
static void gfx_blink_line(gint n_blinks, gint r1, gint c1, gint r2, gint c2)
{
...
void gfx_blink_winner(gint n_blinks)
{
	...
	if (!prefs.do_animate || gnect.winner == -1) {
		return;
	}
...
void gfx_wipe_board(void)
{
	...
	if (!prefs.do_animate || !prefs.do_wipes || gnect.veleng_str[2] == '\0') {
		return;
	}
...
static void gfx_draw_grid(Theme *theme)
{
	GdkColormap *cmap;
	GdkGC *gc;
	GdkColor colour;
	...
	if (!prefs.do_grids || !theme->gridRGB) {
		return;
	}
...
gboolean gfx_load(Theme *theme, const gchar *fname_tileset, const gchar *fname_background)
{
	...
	if (DRAW_AREA_WIDTH != old_width || DRAW_AREA_HEIGHT != old_height) {
		/* avoid momentarily displaying a garbled mess */
