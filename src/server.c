...
#ifdef GGZSPECTATORS
void
game_handle_ggz_spectator_join (GGZdMod * ggz, GGZdModEvent event,
				const void *data)
{
  int i, fd;
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (ggz_write_int (fd, seat.type) < 0)
      return;
    if (seat.type != GGZ_SEAT_OPEN && ggz_write_string (fd, seat.name) < 0)
      return;
...
void
game_handle_ggz_spectator_leave (GGZdMod * ggz, GGZdModEvent event,
				 const void *data)
{
  if (seats_empty ())
...
game_handle_ggz_seat (GGZdMod * ggz, GGZdModEvent event, const void *data)
{
  ...
  if (new_seat.type == GGZ_SEAT_PLAYER) {
    ...
    } else if (connectx_game.state == CONNECTX_STATE_INIT
...
void
game_handle_ggz_player (GGZdMod * ggz, GGZdModEvent event, const void *data)
{
  int num = *(int *) data;
  int fd, op, move;
  GGZSeat seat;
...
void
game_handle_ggz_spectator (GGZdMod * ggz, GGZdModEvent event,
			   const void *data)
{
  int num = *(int *) data;
  int fd, op;
...
static int
game_send_move (int num, int column)
{
  ...
  if (seat.fd != -1) {
    ggzdmod_log (connectx_game.ggz, "Sending player %d's move to player %d",
		 num, opponent);
...
static int
game_read_move (int num, int *move)
{
...
static int
game_req_move (int num)
{
  GGZSeat seat = ggzdmod_get_seat (connectx_game.ggz, num);

  ggzdmod_log (connectx_game.ggz, "Requesting move from player %d on %d", num,
	       seat.fd);

  if (ggz_write_int (seat.fd, CONNECTX_REQ_MOVE) < 0) {
    ggzdmod_log (connectx_game.ggz, "Error requesting move from player %d",
		 num);
    return -1;
  }
...
game_do_move (int move)
{
  ...
  ggzdmod_log (connectx_game.ggz, "Player %d moving in column %d",
	       connectx_game.turn, move);
...
/* Do bot moves */
static int
game_bot_move (int me)
{
  return playgame (vstr, vboard) - 1;
}

/* Check for valid move */
static char
game_check_move (int num, int move)
{
...
int
game_check_tile (int x, int y, int d, int count)
{
  switch (d) {			/* Direction */
  case 0:			/* Left */
...
static int
game_send_options_request (int fd)
{
...
game_send_options (int fd)
{
  ...
  ggzdmod_log (connectx_game.ggz, "\tboardheight=%d",
	       connectx_game.boardheight);
  ggzdmod_log (connectx_game.ggz, "\tboardwidth=%d",
	       connectx_game.boardwidth);
  ggzdmod_log (connectx_game.ggz, "\tconectlength=%d",
	       connectx_game.connectlength);
...
game_read_options (int seat)
{
  ...
  ggzdmod_log (connectx_game.ggz, "\tboardwidth=%d",
	       connectx_game.boardwidth);
  ggzdmod_log (connectx_game.ggz, "\tconnectlength=%d",
	       connectx_game.connectlength);
...
game_handle_newgame (int seat)
{
  ...
  if (ggzdmod_get_seat (connectx_game.ggz, (seat + 1) % 2).type ==
      GGZ_SEAT_BOT)
    connectx_game.play_again++;
  ...
  if (connectx_game.state == CONNECTX_STATE_WAIT
      && connectx_game.play_again == 2 && seats_full ()) {
    connectx_game.turn = 0;
    connectx_game.state = CONNECTX_STATE_PLAYING;
    repeatmove = 0;
    game_move ();
